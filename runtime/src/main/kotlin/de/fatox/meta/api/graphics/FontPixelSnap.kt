 ...
 *
 * [FontProvider]-generated glyph atlases are rasterized at an oversampled device-pixel resolution, so they only stay
 * crisp when drawn at a position that lands on the display pixel grid - anything fractional forces the GPU to
 * bilinearly blend across texel edges, blurring the glyph even though the underlying texture is high resolution.
 * [physicalPixelsPerUnit]
 ...
 * font's own render scale (it is rasterized at physical pixels multiplied by the atlas oversample, then
