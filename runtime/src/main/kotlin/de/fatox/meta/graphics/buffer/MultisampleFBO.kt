 ...
 * gltexture by [MultisampleFBO.getColorBufferTexture]. This class will only work with OpenGL ES 2.0.
 *
 *
 *
 *
 ...
 */
class MultisampleFBO protected constructor(protected var bufferBuilder: GLFrameBufferBuilder<out MultisampleFBO>) :
    Disposable {
    ...
    protected var nonMultisampledFbo: FrameBuffer? = null
    ...
    fun getColorBufferTexture(): Texture? {
        if (nonMultisampledFbo == null) {
            // TODO fix
            return null
        }
    ...
    fun getTextureAttachments(): Array<Texture> {
        ...
        Gdx.gl30.glBindFramebuffer(GL30.GL_READ_FRAMEBUFFER, framebufferHandle)
        Gdx.gl30.glBindFramebuffer(GL30.GL_DRAW_FRAMEBUFFER, nonMultisampledFbo.getFramebufferHandle())
        val intBuffer: IntBuffer = BufferUtils.newIntBuffer(1)
    ...
    protected fun createTexture(attachmentSpec: FrameBufferTextureAttachmentSpec): Int {
        try {
            val texture: Int = Gdx.gl.glGenTexture()
    ...
    protected fun disposeColorTexture(colorTexture: Texture) {
    ...
    protected fun build() {
        ...
        val imBuilder: GLFrameBuffer.FrameBufferBuilder = GLFrameBuffer.FrameBufferBuilder(getWidth(), getHeight())
        ...
        if (isMRT) {
            val buffer: IntBuffer = BufferUtils.newIntBuffer(colorTextureCounter)
    ...
    override fun dispose() {
        ...
        gl.glDeleteFramebuffer(framebufferHandle)
        if (buffers[Gdx.app] != null) buffers[Gdx.app]!!
            .removeValue(this, true)
        if (nonMultisampledFbo != null) {
    ...
    fun end(
        x: Int = 0,
        y: Int = 0,
        width: Int = Gdx.graphics.getBackBufferWidth(),
        height: Int = Gdx.graphics.getBackBufferHeight()
    ) {
        unbind()
    ...
    private fun checkValidBuilder() {
        val runningGL30: Boolean = Gdx.graphics.isGL30Available()
        if (!runningGL30) {
            if (bufferBuilder.hasPackedStencilDepthRenderBuffer) {
    ...
    abstract class GLFrameBufferBuilder<U : MultisampleFBO?>(var width: Int, var height: Int) {
        ...
        fun addBasicColorTextureAttachment(format: Pixmap.Format?): GLFrameBufferBuilder<U> {
            val glFormat: Int = Pixmap.Format.toGlFormat(format)
            val glType: Int = Pixmap.Format.toGlType(format)
    ...
    companion object {
        /** the frame buffers  */
        protected val buffers: MutableMap<Application, Array<MultisampleFBO>?> = HashMap()
