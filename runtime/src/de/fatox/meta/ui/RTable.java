...
/*******************************************************************************
 * Copyright 2011 See AUTHORS file. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in
...
/**
 * A group that sizes and positions children using table constraints. By default, {@link #getTouchable()} is {@link Touchable#childrenOnly}.
 * <p>
 * The preferred and minimum sizes are that of the children when laid out in columns and rows.
 * @author Nathan Sweet
 */
public class RTable extends WidgetGroup {
    static public Color debugRTableColor = new Color(0, 0, 1, 1);
    static public Color debugRCellColor = new Color(1, 0, 0, 1);
    static public Color debugActorColor = new Color(0, 1, 0, 1);
    static public float virtualHeight = Gdx.graphics.getHeight();
    static public float virtualWidth = Gdx.graphics.getWidth();

    static final Pool<RCell> RCellPool = new Pool<RCell>() {
        @Override
        protected RCell newObject() {
            return new RCell();
        }
    };
    ...
    private final Array<RCell> RCells = new Array(4);
    private final RCell RCellDefaults;
    private final Array<RCell> columnDefaults = new Array(2);
    private RCell rowDefaults;
    ...
    public RTable(Skin skin) {
        this.skin = skin;

        RCellDefaults = obtainRCell();
        RCellDefaults.defaults();

        setTransform(false);
        setTouchable(Touchable.childrenOnly);
    }

    private RCell obtainRCell() {
        RCell RCell = RCellPool.obtain();
        RCell.setLayout(this);
        return RCell;
    }
    ...
    /**
     * Sets the background drawable from the skin and adjusts the table's padding to match the background. This may only be called if
     * {@link RTable#RTable(Skin)} or {@link #setSkin(Skin)} was used.
     * @see #setBackground(Drawable)
     */
    public void setBackground(String drawableName) {
        if (skin == null)
            throw new IllegalStateException("RTable must have a skin set to use this method.");
     ...
     * @see #setBackground(Drawable)
     */
    public RTable background(Drawable background) {
     ...
     */
    public RTable background(String drawableName) {
    ...
    /**
     * Causes the contents to be clipped if they exceed the table actor's bounds. Enabling clipping will set {@link #setTransform(boolean)}
     * to true.
     */
    ...
    /** Adds a new RCell to the table with the specified actor. */
    public <T extends Actor> RCell<T> add(T actor) {
        RCell<T> RCell = obtainRCell();
        RCell.actor = actor;

        Array<RCell> RCells = this.RCells;
        int RCellCount = RCells.size;
        if (RCellCount > 0) {
            // Set RCell column and row.
            RCell lastRCell = RCells.peek();
            if (!lastRCell.endRow) {
                RCell.column = lastRCell.column + lastRCell.colspan;
                RCell.row = lastRCell.row;
            } else {
                RCell.column = 0;
                RCell.row = lastRCell.row + 1;
            }
            // Set the index of the RCell above.
            if (RCell.row > 0) {
                outer: for (int i = RCellCount - 1; i >= 0; i--) {
                    RCell other = RCells.get(i);
                    for (int column = other.column, nn = column + other.colspan; column < nn; column++) {
                        if (column == RCell.column) {
                            RCell.RCellAboveIndex = i;
                            break outer;
                        }
                    }
                }
        ...
        RCell.set(RCellDefaults);
        if (RCell.column < columnDefaults.size) {
            RCell columnRCell = columnDefaults.get(RCell.column);
            if (columnRCell != null)
                RCell.merge(columnRCell);
        }
        RCell.merge(rowDefaults);
    ...
    /** Adds a new RCell with a label. This may only be called if {@link RTable#RTable(Skin)} or {@link #setSkin(Skin)} was used. */
    public RCell<Label> add(String text) {
        if (skin == null)
            throw new IllegalStateException("RTable must have a skin set to use this method.");
    ...
    /** Adds a new RCell with a label. This may only be called if {@link RTable#RTable(Skin)} or {@link #setSkin(Skin)} was used. */
    public RCell<Label> add(String text, String labelStyleName) {
        if (skin == null)
            throw new IllegalStateException("RTable must have a skin set to use this method.");
    ...
    /** Adds a new RCell with a label. This may only be called if {@link RTable#RTable(Skin)} or {@link #setSkin(Skin)} was used. */
    public RCell<Label> add(String text, String fontName, Color color) {
        if (skin == null)
            throw new IllegalStateException("RTable must have a skin set to use this method.");
    ...
    /** Adds a new RCell with a label. This may only be called if {@link RTable#RTable(Skin)} or {@link #setSkin(Skin)} was used. */
    public RCell<Label> add(String text, String fontName, String colorName) {
        if (skin == null)
            throw new IllegalStateException("RTable must have a skin set to use this method.");
        return add(new Label(text, new LabelStyle(skin.getFont(fontName), skin.getColor(colorName))));
    }

    /** Adds a RCell without an actor. */
    public RCell add() {
        return add((Actor) null);
    }

    /**
     * Adds a new RCell to the table with the specified actors in a {@link Stack}.
     * @param actors
     *            May be null to add a stack without any actors.
     */
    public RCell<Stack> stack(Actor... actors) {
    ...
    public boolean removeActor(Actor actor) {
        if (!super.removeActor(actor))
            return false;
        RCell RCell = getRCell(actor);
        if (RCell != null)
            RCell.actor = null;
        return true;
    }

    /** Removes all actors and RCells from the table. */
    @Override
    public void clearChildren() {
        Array<RCell> RCells = this.RCells;
        for (int i = RCells.size - 1; i >= 0; i--) {
            RCell RCell = RCells.get(i);
            Actor actor = RCell.actor;
            if (actor != null)
                actor.remove();
        }
        RCellPool.freeAll(RCells);
        RCells.clear();
        rows = 0;
        columns = 0;
        if (rowDefaults != null)
            RCellPool.free(rowDefaults);
        rowDefaults = null;
    ...
    /**
     * Removes all actors and RCells from the table (same as {@link #clear()}) and additionally resets all table properties and RCell,
     * column, and row defaults.
     */
    public void reset() {
        clear();
        ...
        for (int i = 0, n = columnDefaults.size; i < n; i++) {
            RCell columnRCell = columnDefaults.get(i);
            if (columnRCell != null)
                RCellPool.free(columnRCell);
        }
        columnDefaults.clear();
    }

    /**
     * Indicates that subsequent RCells should be added to a new row and returns the RCell values that will be used as the defaults for all
     * RCells in the new row.
     */
    public RCell row() {
        if (RCells.size > 0) {
            endRow();
    ...
    private void endRow() {
        ...
        for (int i = RCells.size - 1; i >= 0; i--) {
            ...
            if (RCell.endRow)
                break;
            rowColumns += RCell.colspan;
        }
        columns = Math.max(columns, rowColumns);
        rows++;
        RCells.peek().endRow = true;
    }

    /**
     * Gets the RCell values that will be used as the defaults for all RCells in the specified column. Columns are indexed starting at 0.
     */
    public RCell columnDefaults(int column) {
        RCell RCell = columnDefaults.size > column ? columnDefaults.get(column) : null;
        if (RCell == null) {
            RCell = obtainRCell();
            RCell.clear();
            if (column >= columnDefaults.size) {
                ...
                columnDefaults.add(RCell);
            } else
                columnDefaults.set(column, RCell);
        }
        return RCell;
    }

    /** Returns the RCell for the specified actor in this table, or null. */
    public <T extends Actor> RCell<T> getRCell(T actor) {
        Array<RCell> RCells = this.RCells;
        for (int i = 0, n = RCells.size; i < n; i++) {
            RCell c = RCells.get(i);
            if (c.actor == actor)
                return c;
        }
        return null;
    ...
    /** Returns the RCells for this table. */
    public Array<RCell> getRCells() {
        return RCells;
    }
    ...
    /** The RCell values that will be used as the defaults for all RCells. */
    public RCell defaults() {
        return RCellDefaults;
    }

    /** Sets the padTop, padLeft, padBottom, and padRight around the table to the specified value. */
    public RTable pad(Value pad) {
        if (pad == null)
    ...
    public RTable pad(Value top, Value left, Value bottom, Value right) {
        if (top == null)
    ...
    public RTable padTop(Value padTop) {
        if (padTop == null)
    ...
    public RTable padLeft(Value padLeft) {
        if (padLeft == null)
    ...
    public RTable padBottom(Value padBottom) {
        if (padBottom == null)
    ...
    public RTable padRight(Value padRight) {
        if (padRight == null)
    ...
    /** Sets the padTop, padLeft, padBottom, and padRight around the table to the specified value. */
    public RTable pad(float pad) {
        pad(new Fixed(pad));
        return this;
    }

    public RTable pad(float top, float left, float bottom, float right) {
    ...
    /** Padding at the top edge of the table. */
    public RTable padTop(float padTop) {
    ...
    /** Padding at the left edge of the table. */
    public RTable padLeft(float padLeft) {
    ...
    /** Padding at the bottom edge of the table. */
    public RTable padBottom(float padBottom) {
    ...
    /** Padding at the right edge of the table. */
    public RTable padRight(float padRight) {
    ...
    @Override
    public RTable debugAll() {
        super.debugAll();
        return this;
    }

    /** Turns on table debug lines. */
    public RTable debugRTable() {
        super.setDebug(true);
    ...
    /** Turns on RCell debug lines. */
    public RTable debugRCell() {
        super.setDebug(true);
        if (debug != Debug.RCell) {
            this.debug = Debug.RCell;
            invalidate();
        }
        return this;
    }
    ...
    public RTable debug(Debug debug) {
    ...
    /**
     * Returns the row index for the y coordinate, or -1 if there are no RCells.
     * @param y
    ...
    public void layout() {
        ...
        // Validate children separately from sizing actors to ensure actors without a RCell are validated.
    ...
    private void computeSize() {
        ...
        if (RCellCount > 0 && !RCells.peek().endRow)
            endRow();
        ...
        for (int i = 0; i < RCellCount; i++) {
            ...
            // Compute combined padding/spacing for RCells.
            ...
            if (c.RCellAboveIndex != -1) {
                RCell above = RCells.get(c.RCellAboveIndex);
            ...
            // Determine minimum and preferred RCell sizes.
        ...
        outer: for (int i = 0; i < RCellCount; i++) {
            RCell c = RCells.get(i);
        ...
        // Distribute any additional min and pref width added by colspanned RCells to the columns spanned.
        for (int i = 0; i < RCellCount; i++) {
            RCell c = RCells.get(i);
        ...
        // Size uniform RCells to the same width/height.
    ...
    /**
     * Positions and sizes children of the table using the RCell associated with each child. The values given are the position within the
     * parent and size of the table.
     */
    private void layout(float layoutX, float layoutY, float layoutWidth, float layoutHeight) {
        ...
        if (totalGrowWidth == 0)
            ...
            columnWeightedWidth = RTable.columnWeightedWidth = ensureSize(RTable.columnWeightedWidth, columns);
        ...
        if (totalGrowHeight == 0)
            ...
            rowWeightedHeight = RTable.rowWeightedHeight = ensureSize(RTable.rowWeightedHeight, rows);
        ...
        // Determine actor and RCell sizes (before expand or fill).
        for (int i = 0; i < RCellCount; i++) {
            RCell c = RCells.get(i);
        ...
        // Distribute any additional width added by colspanned RCells to the columns spanned.
        for (int i = 0; i < RCellCount; i++) {
            RCell c = RCells.get(i);
            int colspan = c.colspan;
            if (colspan == 1)
        ...
        // Position actors within RCells.
        float currentX = x, currentY = y;
        for (int i = 0; i < RCellCount; i++) {
            RCell c = RCells.get(i);

            float spannedRCellWidth = 0;
            for (int column = c.column, nn = column + c.colspan; column < nn; column++)
                spannedRCellWidth += columnWidth[column];
            spannedRCellWidth -= c.computedPadLeft + c.computedPadRight;

            currentX += c.computedPadLeft;

            float fillX = c.fillX, fillY = c.fillY;
            if (fillX > 0) {
                c.actorWidth = Math.max(spannedRCellWidth * fillX, c.minWidth.get(c.actor));
            ...
            if ((align & Align.left) != 0)
                ...
                c.actorX = currentX + spannedRCellWidth - c.actorWidth;
            else
                c.actorX = currentX + (spannedRCellWidth - c.actorWidth) / 2;
            ...
            if (c.endRow) {
                ...
                currentX += spannedRCellWidth + c.computedPadRight;
        }
        ...
        if (debug == Debug.table || debug == Debug.all) {
            addDebugRect(layoutX, layoutY, layoutWidth, layoutHeight, debugRTableColor);
            addDebugRect(x, y, tableWidth - hpadding, tableHeight - vpadding, debugRTableColor);
        }
        for (int i = 0; i < RCellCount; i++) {
            RCell c = RCells.get(i);
            ...
            spannedRCellWidth -= c.computedPadLeft + c.computedPadRight;
            currentX += c.computedPadLeft;
            if (debug == Debug.RCell || debug == Debug.all) {
                addDebugRect(currentX, currentY + c.computedPadTop, spannedRCellWidth, rowHeight[c.row] - c.computedPadTop - c.computedPadBottom,
                        debugRCellColor);
            }
    ...
    static public Value backgroundTop = new Value() {
        ...
        public float get(Actor context) {
            Drawable background = ((RTable) context).background;
