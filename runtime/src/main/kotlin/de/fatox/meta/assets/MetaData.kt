...
class MetaData(root: FileHandle? = null) {
	 ...
	 *
	 * Not carried, deliberately: ownership, creation time, Windows DOS attributes and any extended attributes. A game's
	 * save file is not hidden or system, and its save directory belongs to one user, so there is nothing there worth
	 * the code to preserve. Anything added to that list belongs in the table above, with a test.
	 */
